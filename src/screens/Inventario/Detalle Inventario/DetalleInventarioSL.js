...
import { View, Text, FlatList, ActivityIndicator, ScrollView, StyleSheet, TouchableOpacity, useWindowDimensions, Alert, TouchableHighlight } from 'react-native';
...
export const DetalleInventarioSL = ({ navigation, route }) => {
    ...
    const { url, tokenInfo, fetchDataDetalleInvSL, isLoading, setIsLoading, data, setData, dataComplete, searchDetalleInvSL, setSearchDetalleInvSL, setModuloScan, handleSearchDetalleInvSL } = useContext(AuthContext);
    ...
    const renderItem = ({ item, index }) => (
        <View style={{ padding: 1, borderBottomWidth: 1, borderBottomColor: '#ccc', flexDirection: 'row', paddingRight: 20 }}>
            <CheckBox
            ...
            <List.Accordion
                ...
                <View style={{ marginVertical: 10 }}>
                    <Text style={{ ...styles.tituloListItem, fontSize: windowsWidth > 500 ? 24 : 20 }}>Código de articulo:    <Text style={styles.tituloListItemData}>{item.ItemCode}</Text></Text>
                    <Text style={{ ...styles.tituloListItem, fontSize: windowsWidth > 500 ? 24 : 20 }}>{item.GestionItem == 'L' ? 'Lote' : 'Serie'}:    <Text style={styles.tituloListItemData}>{item.IdCode}</Text></Text>
                    <Text style={{ ...styles.tituloListItem, fontSize: windowsWidth > 500 ? 24 : 20 }}>Cantidad disponible:    <Text style={styles.tituloListItemData}>{item.QuantityDisp}</Text></Text>
                    ...
                    <Text style={{ ...styles.tituloListItem, fontSize: windowsWidth > 500 ? 24 : 20 }}>En fecha:    <Text style={styles.tituloListItemData}>{item.InDate}</Text></Text>
                </View>
            </List.Accordion>
        </View>
    );
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <View style={{ flexDirection: 'row' }}>
                <View style={{ flex: .6, alignItems: 'flex-start', flexDirection: 'row', paddingLeft: '5%' }}>
                    {selectedItems.length > 0 ?
                        <Text onPress={() => {
                            Alert.alert('Advertencia', '¿Estas seguro de quitar de la lista de seleccion?', [,
                                {
                                    text: 'Si', onPress: () => {
                                        setItemsSeleccionados([]);
                                        setSelectedItems([])
                            ...
                            style={{ alignSelf: 'center', fontSize: windowsWidth > 500 ? 20 : 16, textDecorationLine: 'underline' }}>Quitar seleccion
                        </Text> : ''
                    }
                </View>

                <View style={{ flexDirection: 'row', justifyContent: 'flex-end', paddingHorizontal: 20, alignItems: 'center', flex: .6 }}>
                    <Text style={{ fontSize: 20, marginHorizontal: 20 }}>{itemsPerPage * currentPage - 19 + '-' + itemsPerPage * currentPage} de {data.length}</Text>
                    <Icon
            ...
            <Modal isVisible={isModalImpresionEtiquetas} style={{ flex: 1 }} animationInTiming={1000}>
                <View style={{ backgroundColor: '#fff', borderRadius: 10, padding: windowsWidth > 500 ? 40 : 15, margin: windowsWidth > 500 ? '10%' : '1%', flexDirection: 'col', height: '100%' }}>
                    <Text style={{ fontSize: 26, textAlign: 'center', margin: 20, gap: 20 }}>Impresion Etiquetas</Text>
                    <ScrollView>
                        <View style={{ flex: 1, padding: 20 }}>

                            {itemsSeleccionados.map((item, index) =>
                                <Card key={index} containerStyle={{ backgroundColor: '#f5f8fa', borderRadius: 20, flexDirection: 'column', height: 'auto' }}>
                                    <Badge
                                        status="success"
                                    ...
                                    />
                                    <Card.Title style={{ fontSize: windowsWidth > 500 ? 30 : 24, color: '#000', fontWeight: 'bold' }}>{item.IdCode}</Card.Title>
            ...
            <Modal isVisible={isModalCantidad} style={{ flex: 1 }} animationInTiming={1000}>
                <View style={{ backgroundColor: '#fff', borderRadius: 10, padding: 40, margin: '10%', flexDirection: 'col' }}>
                    <Text style={{ fontSize: 26, textAlign: 'center', margin: 20, gap: 20 }}>Ingresa Cantidad {itemSeleccionado}</Text>
                    <View style={{ flexDirection: 'row' }}>
            ...
            <Modal isVisible={isModalImpresionSL} style={{ flex: 1 }} animationInTiming={1000}>
                <View style={{ backgroundColor: '#fff', borderRadius: 10, padding: 40, margin: '10%', flexDirection: 'col' }}>
                    <Text style={{ fontSize: 26, textAlign: 'center', margin: 20, gap: 20 }}>Impresion Series y Lotes</Text>
                    <ScrollView>
                        <View style={{ flex: 1, padding: 20 }}>

...
import { View, Text, FlatList, ActivityIndicator, ScrollView, StyleSheet, TouchableOpacity, useWindowDimensions, Alert, TouchableHighlight, PixelRatio } from 'react-native';
...
export const DetalleInventario = ({ navigation }) => {
    ...
    const { url, tokenInfo, setModuloScan, dataDetalleInv, setDataDetalleInv, abrirDetalleInvSL, data, paramsDetalleInvSL, setParamsDetalleInvSL, dataComplete,
        fetchDataDetalleInvSL, isLoading, setIsLoading, searchDetalleInv, setSearchDetalleInv, handleSearchDetalleInv, dataCompleteDI, setDataCompleteDI, fetchDataDetalleInv,
        setData, setDataComplete, setSearchDetalleInvSL } = useContext(AuthContext);
    ...
    const [isModalImpresionEtiquetas, setIsModalImpresionEtiquetas] = useState(false)
    ...
    useEffect(() => {
        if (paramsDetalleInvSL.length > 0) {
            ...
            fetchDataDetalleInvSL(paramsDetalleInvSL[0].ItemCode, paramsDetalleInvSL[0].WhsCode, paramsDetalleInvSL[0].GestionItem, paramsDetalleInvSL[0].IdCode, paramsDetalleInvSL[0].conEscaner)
        }
    }, [paramsDetalleInvSL])
    ...
    const handleCheckboxToggle = (itemId, item) => {
        ...
        if (dato.length > 0) {
            ...
            const updatedArray = itemsSeleccionados.filter(element => element.Id !== item.Id);
    ...
    const handleFloatingButtonPress = (accion) => {
        if (accion == 'print') {
            ...
            if (itemsSeleccionados.length > 0) {
                ...
                Alert.alert('Info', 'No ha seleccionado ningun item', [,
                    { text: 'OK', },
                ])
            }
        } else {
    ...
    const handleSubmit = () => {
        ...
        fetchDataDetalleInvSL(codigoArticulo, almacen, gestionA, idCodeSerieLote, true)
    ...
    const renderItem = ({ item, index }) => (
        <View style={{ padding: 1, borderBottomWidth: 1, borderBottomColor: '#ccc', flexDirection: 'row', paddingRight: item.GestionItem == 'I' ? 20 : 0 }}>
            {item.GestionItem == 'I' ?
                < CheckBox
            ...
            <List.Accordion
                ...
                <View style={{ marginVertical: 10, flexDirection: 'row', width: '100%' }}>
                    <View style={{ flex: .8 }}>
                        <Text style={{ ...styles.tituloListItem, fontSize: windowsWidth > 500 ? 24 : 20 }}>Código de articulo producto:    <Text style={styles.tituloListItemData}>{item.CodeBars}</Text></Text>
                        <Text style={{ ...styles.tituloListItem, fontSize: windowsWidth > 500 ? 24 : 20 }}>Nombre del articulo:    <Text style={styles.tituloListItemData}>{item.ItemName}</Text></Text>
                        <Text style={{ ...styles.tituloListItem, fontSize: windowsWidth > 500 ? 24 : 20 }}>Almacen:    <Text style={styles.tituloListItemData}>{item.WhsCode}</Text></Text>
                        <Text style={{ ...styles.tituloListItem, fontSize: windowsWidth > 500 ? 24 : 20 }}>En Stock:    <Text style={styles.tituloListItemData}>{item.OnHand}</Text></Text>
                        <Text style={{ ...styles.tituloListItem, fontSize: windowsWidth > 500 ? 24 : 20 }}>Disponible:    <Text style={styles.tituloListItemData}>{item.StckDsp}</Text></Text>
                        <Text style={{ ...styles.tituloListItem, fontSize: windowsWidth > 500 ? 24 : 20 }}>Unidad de medida:    <Text style={styles.tituloListItemData}>{item.InvntryUom}</Text></Text>
                        {item.GestionItem == 'I' ? '' :
                            <Text style={{ ...styles.tituloListItem, fontSize: windowsWidth > 500 ? 24 : 20 }}>Gestion del articulo:    <Text style={styles.tituloListItemData}>{item.GestionItem == 'S' ? 'Serie' : 'Lote'}</Text></Text>
                        }
                    </View>
                    <View style={{ justifyContent: 'center', flex: .2, alignItems: 'center' }}>
                        {item.GestionItem != 'I' ?
                            <>
                                ...
                                <Text style={{ color: '#000000', textAlign: 'center', fontWeight: 600, fontSize: windowsWidth > 500 ? 16 : 12 }}> {item.GestionItem == 'S' ? 'Ver Series' : 'Ver Lotes'}</Text>
                            </>
                            : ''
                        }
                    </View>
        ...
        <View style={{ flex: 1 }}>
            ...
            <View style={{ flexDirection: 'row', paddingHorizontal: 10, columnGap: 5 }}>
                <SearchBar
                    ...
                    inputStyle={{ backgroundColor: '#ffffff', borderRadius: 10, fontSize: windowsWidth > 500 ? 22 : 18 }}
                    containerStyle={{ backgroundColor: '#ffffff', borderRadius: 50, marginVertical: 20, padding: 0, borderColor: '#ffffff', flex: 1 }}
                    theme
                />
                {/* {tune == true ?
                    <Icon
            ...
            {buscarCategoria == true ?
                <View style={{ flexDirection: 'row', justifyContent: 'center', gap: 10 }}>
                    <CheckBox
                        ...
                        onValueChange={(value) => { setOpcion1Seleccionada(value); setOpcion2Seleccionada(value); setOpcion3Seleccionada(value) }}
                    />
            ...
            <View style={{ flexDirection: 'row' }}>
                <View style={{ flex: .6, alignItems: 'flex-start', flexDirection: 'row', paddingLeft: 5 }}>
                    {selectedItems.length > 0 ?
                        <>
                            <Icon
                                ...
                                containerStyle={{ alignSelf: 'center' }}
                                onPress={() => {
                                    Alert.alert('Advertencia', '¿Estas seguro de eliminar selección?', [,
                                        {
                                            text: 'Si', onPress: () => {
                                                setItemsSeleccionados([]);
                                                setSelectedItems([])
                            ...
                            <Text onPress={() => {
                                ...
                                style={{ alignSelf: 'center', fontSize: windowsWidth > 500 ? 20 : 16, textDecorationLine: 'underline' }}>Eliminar seleccion
                            </Text></>
                        : ''
                    }
                </View>
                <View style={{ flexDirection: 'row', justifyContent: 'flex-end', paddingHorizontal: 20, alignItems: 'center', flex: .6 }}>
                    <Text style={{ fontSize: windowsWidth > 500 ? 20 : 16, marginHorizontal: 20 }}>{itemsPerPage * currentPage - 19 + '-' + itemsPerPage * currentPage} de {dataDetalleInv.length}</Text>
                    <Icon
            ...
            <Modal isVisible={isModalImpresionEtiquetas} style={{ flex: 1 }} animationInTiming={1000}>
                <View style={{ backgroundColor: '#ffffff', borderRadius: 10, padding: windowsWidth > 500 ? 40 : 15, margin: windowsWidth > 500 ? '10%' : '5%', flexDirection: 'col', height: '100%' }}>
                    <Text style={{ fontSize: 26, textAlign: 'center', margin: 20, gap: 20 }}>Impresion Etiquetas</Text>
                    <ScrollView>
                        <View style={{ flex: 1, padding: 20 }}>

                            {itemsSeleccionados.map((item, index) =>
                                <Card key={index} containerStyle={{ backgroundColor: '#f5f8fa', borderRadius: 20, flexDirection: 'column', height: 'auto', marginBottom: 20 }}>
                                    <Badge
                                        status="success"
                                        value={item.cantidadEtiquetas}
                                        containerStyle={{ position: 'absolute', top: -35, right: -25 }}
                                    ...
                                    />
                                    <Card.Title style={{ fontSize: windowsWidth > 500 ? 30 : 24, color: '#000000', fontWeight: 'bold' }}>{item.ItemCode}</Card.Title>
                    ...
                    <View style={{ marginTop: 20, rowGap: 10 }}>
                        ...
                        <Button
                            ...
                            onPress={() => {
                                Alert.alert('Advertencia', '¿Estas seguro de eliminar los elementos seleccionados?', [,
                                    {
            ...
            <Modal isVisible={isModalCantidad} style={{ flex: 1 }} animationInTiming={1000}>
                <View style={{ backgroundColor: '#fff', borderRadius: 10, padding: 40, margin: '10%', flexDirection: 'col' }}>
                    <Text style={{ fontSize: 26, textAlign: 'center', margin: 20, gap: 20 }}>Ingresa Cantidad</Text>
                    <View style={{ flexDirection: 'row' }}>

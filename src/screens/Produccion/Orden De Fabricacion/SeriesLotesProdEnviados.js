...
export const SeriesLotesProdEnviados = ({ navigation, route }) => {
    ...
    const { isLoading, setSerieLoteTransfer, tablaSeriesLotesTransfer, dataSLProdEnviado, setDataSLProdEnviado, eliminarSLProdEnviado } = useContext(AuthContext);
                    ...
                    <ScrollView>
                        <DataTable>
                            <DataTable.Header style={{ backgroundColor: '#00913f' }}>
                                <DataTable.Title textStyle={styles.cellTitle} style={styles.cellContent}>{route.params.gestionItem == 'S' ? 'N° Serie' : 'N° Lote'}</DataTable.Title>
                                ...
                                {route.params.gestionItem == 'L' ? <DataTable.Title textStyle={styles.cellTitle} style={[styles.cellContent, styles.numericCell]}>Cantidad</DataTable.Title> : ''}
                                            ...
                                            Alert.alert('Advertencia', '¿Estas seguro de eliminar el elemento seleccionado?', [
                                                ...
                                                },
                                                {
                                                    text: 'Eliminar', onPress: () => {
                                                        eliminarSLProdEnviado(index, route.params, item)
                                                    }
                                                },
                                            ]);
                                        }} /></Text></View>
                            ...
                            <DataTable.Pagination
                                page={page}
                                numberOfPages={Math.ceil(dataSLProdEnviado.length / itemsPerPage)}

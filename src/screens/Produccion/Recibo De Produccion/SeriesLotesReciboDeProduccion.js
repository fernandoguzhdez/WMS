...
import { View, StyleSheet, Text, ScrollView, Alert, useWindowDimensions, TextInput } from 'react-native';
...
export function SeriesLotesReciboDeProduccion({ navigation, route }) {
    ...
    const { isLoading, dataSLReciboProd, displayForm, setDisplayform, setValueSLReciboProd, filtroDataSLReciboProd, setFiltroDataSLReciboProd, setDataSLReciboProd, eliminarSLRecProd,
        isModalSLReciboProd, setIsModalSLReciboProd, itemSelectRecProd, setItemSelectRecProd, visibleFormCantidad, setVisibleFormCantidad, artSelectRecProd, setArtSelectRecProd } = useContext(AuthContext);
    ...
    useEffect(() => {
        console.log('resultado resta...', artSelectRecProd)
    }, [artSelectRecProd]);
    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#fff', borderRadius: 10 }}>
            ...
            <View style={{ margin: windowsWidth > 500 ? 20 : 0 }}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}>
                    <Icon
                        ...
                        containerStyle={{ width: 'auto', marginBottom: 10, display: displayForm == false ? 'flex' : 'none' }}
                        iconStyle={{ color: '#ffff', backgroundColor: '#3b5998', width: '100%', height: '100%', textAlign: 'center', textAlignVertical: 'center' }}
                        onPress={() => {
                            setDisplayform(!displayForm)
                        }} />
                </View>
                <View style={{ display: displayForm == false && artSelectRecProd.gestionItem != 'I' ? 'none' : 'flex' }}>
                    <ComponenteCantidad itemSeleccionado={artSelectRecProd} tipo='insertarSL' />
                </View>
                    ...
                    <ScrollView>
                        <DataTable>
                            <DataTable.Header style={{ backgroundColor: '#00913f' }}>
                                <DataTable.Title textStyle={styles.cellTitle} style={styles.cellContent}>{artSelectRecProd.gestionItem == 'S' ? 'N° Serie' : 'N° Lote'}</DataTable.Title>
                                ...
                                <DataTable.Title textStyle={styles.cellTitle} style={[styles.cellContent, styles.numericCell]}>Tipo Clase</DataTable.Title>
                                {artSelectRecProd.gestionItem == 'L' ? <DataTable.Title textStyle={styles.cellTitle} style={[styles.cellContent, styles.numericCell]}>Cantidad</DataTable.Title> : ''}
                            ...
                            {filtroDataSLReciboProd.slice(from, to).map((item, index) => (
                                <DataTable.Row key={index}>
                                    ...
                                    <DataTable.Cell textStyle={{ fontSize: 22, flex: 1, flexWrap: 'wrap', minHeight: 60 }} style={{ maxWidth: 120, justifyContent: 'flex-start', alignItems: 'center' }}><View><Text style={styles.cellContent}>{item.whsCode}</Text></View></DataTable.Cell>
                                    <DataTable.Cell textStyle={{ fontSize: 22, flex: 1, flexWrap: 'wrap', minHeight: 60 }} style={{ maxWidth: 120, justifyContent: 'flex-start', alignItems: 'center' }}><View><Text style={styles.cellContent}>{item.binEntry}</Text></View></DataTable.Cell>
                                    <DataTable.Cell textStyle={{ fontSize: 22, flex: 1, flexWrap: 'wrap', minHeight: 60 }} style={{ maxWidth: 120, justifyContent: 'flex-start', alignItems: 'center' }}><View><Text style={styles.cellContent}>{item.ClaseOp}</Text></View></DataTable.Cell>
                                    ...
                                    <DataTable.Cell textStyle={{ fontSize: 22, flex: 1, flexWrap: 'wrap', minHeight: 60 }} style={{ maxWidth: 120, justifyContent: 'flex-start', alignItems: 'center' }}>
                                        <View>
                                                        ...
                                                        Alert.alert('Advertencia', '¿Estas seguro de eliminar el elemento seleccionado?', [
                                                            ...
                                                            },
                                                            {
                                                                text: 'Eliminar', onPress: () => {
                                                                    eliminarSLRecProd(index, artSelectRecProd, item)
                                                                }
                                                            },
                                                        ]);
                                                    }} />
                            ...
                            <DataTable.Pagination
                                page={page}
                                numberOfPages={Math.ceil(filtroDataSLReciboProd.length / itemsPerPage)}
            ...
            <Overlay isVisible={visibleFormCantidad} onBackdropPress={toggleOverlay} overlayStyle={{ width: '50%', height: 'auto' }}>
                <Text style={{ fontSize: 26, textAlign: 'center', margin: 10 }}>Editar Elemento</Text>
                <ComponenteCantidad articulo={artSelectRecProd} itemSeleccionado={itemSelectRecProd} tipo='Edicion' />
